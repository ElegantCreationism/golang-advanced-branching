...
// All names in the struck are lowercase as will will create extractor methods
type car struct {
	model string
	make string
	typeVehicle string
...
func showRating(model string)  {
	...
	for m, r := range vehicleResult{
		if m == model{
			fmt.Printf("Total ratings:%v\tPositive:%v\tNegative:%v\tNeutral:%v", r.feedbackTotal,
				r.feedbackPositive, r.feedbackNegative, r.feedbackNeutral)
			ratingFound = true
		}
	}
	if !ratingFound {
